...
public class Actions {
	...
	public static void drawTrains(Game game, State state, Graphics2D g2) {
		for (Player player : game.getPlayers()) {
			...
			if (activeTrain != null) {
				...
				java.awt.Point currentPosition = mapPointToScreenPoint(
						activeTrain.getCurrentPosition(), state);
	...
	public static void drawTrainPathOverlay(Player player, State state,
			Graphics2D g2) {
		ActiveTrain activeTrain = player.getActiveTrain();
		if (activeTrain == null)
			return;
		if (activeTrain.getRemainderOfRoute().isEmpty())
			return;

		Track currentTrack = activeTrain.getRemainderOfRoute().get(0);
		// We only want to paint the part of the track left to cover.
		Track partialTrackToPaint = new Track(activeTrain.getCurrentPosition(),
				activeTrain.getFacing());
	...
	private static void drawTrackOverlay(Track track, Color color, State state,
			Graphics2D g2) {
		double zoom = state.getZoom();
		double trackLength = Utilities.length(track);
		Point closestEndToOrigin = Utilities.closestPoint(new Point(0d, 0d),
				track.getPoints());
		java.awt.Point closestEndToOriginScreenPoint = mapPointToScreenPoint(
				closestEndToOrigin, state);
		Point furthestEndToOrigin = track.getOtherPoint(closestEndToOrigin);
		List<Double> vector = Utilities.getVector(closestEndToOrigin,
				furthestEndToOrigin);
		double angle = Math.atan2(-vector.get(1), vector.get(0));

		BufferedImage scaledRailAndWood = state.getScaledRailAndWood();
		Rectangle2D rectangle = new Rectangle((int) (zoom * trackLength),
				(int) (scaledRailAndWood.getHeight() * 0.6));

		Color transparentColour = new Color(color.getRed(), color.getGreen(),
				color.getBlue(), 76);
		g2.setColor(transparentColour);

		g2.translate(closestEndToOriginScreenPoint.getX(),
				closestEndToOriginScreenPoint.getY());
		g2.rotate(-angle);
	...
	public static void drawIntersectionOverlay(Intersection intersection,
			State state, Graphics2D g2) {
		Point intersectionPoint = intersection.getPoint();
		java.awt.Point scaledIntersectionPoint = mapPointToScreenPoint(
				intersectionPoint, state);
		int distanceFromIntersection = 6;
		double zoom = state.getZoom();

		for (Track track : intersection.getActiveConnection()) {
			List<Double> vector = Utilities.getVector(intersectionPoint,
					track.getOtherPoint(intersectionPoint));
			double angle = Math.atan2(-vector.get(1), vector.get(0));

			int triangleHeight = (int) (state.getScaledRailAndWood()
					.getHeight() * 0.8);
			...
			Color transparentColour = new Color(Color.GREEN.getRed(),
					Color.GREEN.getGreen(), Color.GREEN.getBlue(), 220);
			g2.setColor(transparentColour);

			g2.translate(scaledIntersectionPoint.getX(),
					scaledIntersectionPoint.getY());
			g2.rotate(-angle);
	...
	public static void drawMap(Game game, BufferedImage background,
			State state, Graphics2D g2) {
		double zoom = state.getZoom();
		Map map = game.getMap();
	...
	private static void drawTrack(Track track, State state, Graphics2D g2) {
		...
		BufferedImage scaledRailAndWood = state.getScaledRailAndWood();
		Rectangle2D rectangle = new Rectangle((int) (trackLength * zoom),
				scaledRailAndWood.getHeight());

		TexturePaint texturePaint = new TexturePaint(scaledRailAndWood,
				new Rectangle(scaledRailAndWood.getWidth(),
						scaledRailAndWood.getHeight()));
	...
	private static void drawLocations(Game game, State state, Graphics2D g2) {
		...
		for (Player player : game.getPlayers()) {
			...
			if (activeTrain == null)
				continue;
			Location destinationLocation = map.getLocationFromPoint(activeTrain
					.getDestination());
			drawDestinationLocation(destinationLocation, player, state, g2);
		}
	}

	private static void drawRegularLocation(Location location, State state,
			Graphics2D g2) {
		g2.setColor(Color.WHITE);
		double locationSize = state.getZoom() * state.getFlagSize() * 0.5;
		g2.setStroke(new BasicStroke((float) locationSize / 4));
		java.awt.Point locationPoint = mapPointToScreenPoint(
				location.getPoint(), state);
		Rectangle2D.Double rectangle = new Rectangle2D.Double(
				locationPoint.getX() - locationSize / 2, locationPoint.getY()
						- locationSize / 2, locationSize, locationSize);
		g2.draw(rectangle);
	...
	private static void drawDestinationLocation(Location location,
			Player player, State state, Graphics2D g2) {
	...
	public static Point screenPointToMapPoint(java.awt.Point clickPoint,
			State state) {
	...
	public static void selectIntersectionOrTrack(Game game,
			Point mapMousePoint, Double minPickUpDistance, State state) {
		// Priority = Intersection > Tracks

		Map map = game.getMap();

		// Decreased pickup distance for intersections as they have priority
		// over tracks
		Intersection intersection = selectIntersection(map, mapMousePoint,
				minPickUpDistance * 0.7);
	...
	public static Intersection selectIntersection(Map map, Point clickPoint,
	...
	public static Track selectTrack(Map map, Point clickPoint,
			Double minPickUpDistance) {
